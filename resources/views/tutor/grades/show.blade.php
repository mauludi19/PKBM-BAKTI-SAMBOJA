...
<head>
    ...
    <title>Detail Nilai - PKBM Bakti Samboja</title>
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <h1 class="mt-3 text-2xl font-semibold tracking-tight">Detail Nilai</h1>
            </div>
            <a href="{{ route('tutor.grades.edit', $grade) }}" class="rounded-md bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-800">Edit</a>
        </div>

        <div class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <dl class="grid gap-5 sm:grid-cols-2">
                <div><dt class="text-sm text-slate-500">Siswa</dt><dd class="mt-1 font-semibold">{{ $grade->student?->user?->name ?? '-' }}</dd></div>
                ...
                <div class="sm:col-span-2"><dt class="text-sm text-slate-500">Catatan</dt><dd class="mt-1 leading-6 text-slate-700">{{ $grade->notes ?? '-' }}</dd></div>
            </dl>
        </div>
    </main>
</body>
